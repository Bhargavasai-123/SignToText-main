...
def gen_frames():
    ap = Application()
    ap.__init__()  # generate frame by frame from the camera
    while True:
        success, frame = ap.vs.read()
        ret, buffer = cv2.imencode('.jpg', cv2.resize(frame, (400,400)))
        frame = buffer.tobytes()
        ap.video_loop()
# 640, 480
        # Update the global words dictionary
        words["word"] = ap.str
        words["word1"] = ap.word1
        words["word2"] = ap.word2
        words["word3"] = ap.word3
        words["word4"] = ap.word4
...
@app.route('/get_words')
def get_words():
    # Return the global words dictionary as JSON
